...
interface validateAddingCommandType {
  xyte: Page;
  device: string;
  friendlyName: string;
  description: string;
...
let deviceID: string;
export const validateAddingCommand = async ({
  xyte,
  device,
  friendlyName,
  description,
  nameSentToDevice,
}: validateAddingCommandType): Promise<{deviceID: string; modalID: string}> => {
  ...
  await test.step('click on create button', async () => {
    ...
    const dataGetNetwork: [deviceModelsObjectType] = await response.json();
    deviceID = dataGetNetwork.find(
      (value) => value.friendly_name === friendlyName && value.id
    )?.id as string;
  });

  await expect
    .soft(xyte.getByRole('alertdialog', {name: /^Custom command added$/i}))
    .toBeVisible();

  await xyte.getByPlaceholder(/^Search...$/i).type(friendlyName);

  await expect(
    xyte.getByRole('cell', {name: new RegExp(`^${friendlyName}$`, 'i')})
  ).toBeVisible();

  return {deviceID, modalID};
};
